...
// we add the encryption struct from the Goframework/helpers.go
// we add it here has we will use it in our handlers
// but i could add it in any other package, just add the func to any receiver
func (h *Handlers) encrypt(text string) (string, error) {
	enc := goframework.Encryption{Key: []byte(h.App.EncryptionKey)}
