...
type Cache interface {
	...
	Set(string, interface{}, ...int) error // ...int is a optional expiry date
	Forget(string) error
...
type RedisCache struct {
	Conn   *redis.Pool
	Prefix string // will set a unique prefix on all entry(avoid to instance to set the same)
}
...
func (c *RedisCache) Set(str string, value interface{}, expires ...int) error {
	...
	if len(expires) > 0 {
		_, err = redis.Bytes(conn.Do("SETEX", key, expires[0], string(encoded)))
		if err != nil {
			return err
		}

	} else {
		_, err = redis.Bytes(conn.Do("SET", key, string(encoded)))
		if err != nil {
			return err
		}
	}
